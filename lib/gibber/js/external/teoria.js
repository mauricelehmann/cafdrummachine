...
// =======================================
// 
// Teoria is a simple framework for doing music theory related stuff in JavaScript.
// It's dead-simple and consists of very dynamic objects that can interact with eachother in many ways.
//
...
(function() {
    ...
    TeoriaNote.prototype = {
        ...
        enharmonics: function() {
            var enharmonics = [],
                key = this.key(),
                upper = this.interval('m2', 'up'),
                lower = this.interval('m2', 'down');
    ...
    function TeoriaChord(root, name) {
        ...
        var c, code, strQuality, extensions = [],
            i, length, seventh = false,
            parsing = 'quality',
            sharp = false,
            flat = false,
            five = null;
    ...
    teoria.frequency = {
        note: function(fq, concertPitch) {
            ...
            return {
                note: new TeoriaNote(name + (octave + 1)),
                cents: cents
            };
        }
    };
    ...
    teoria.interval.between = function(from, to) {
        var fromKey = from.key(),
            toKey = to.key(),
            semitones, interval, intervalInt, tmp, simpleName, quality;
